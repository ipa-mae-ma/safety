...
class TestGymEnv:
    ...
    def test_plot(self):
        """
        test matplotlib plot function for value function and corresponding policy
        """
        # setup environment
        env = FrozenLakeEnv()
        GAMMA_VI = 0.95
        ...
        # perform value iteration
        Vs_VI, pis_VI = mc.value_iteration(mdp, gamma=GAMMA_VI, nIt=20)
        mc.plot_map(Vs_VI, pis_VI, env=FrozenLakeEnv(), pdf_name='ci/ci_test.pdf')
        print('"ci_test.pdf" file should be generated by function "test_plot"')
        assert os.path.exists(os.path.abspath('.') + '/ci/ci_test.pdf') == True
